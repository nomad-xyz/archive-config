...
static DEVELOPMENT_JSON: &str = include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/configs/development.json"));
static STAGING_JSON: &str = include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/configs/staging.json"));
static PRODUCTION_JSON: &str = include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/configs/production.json"));
...
pub fn get_builtin(name: &str) -> Option<NomadConfig> {
    BUILTINS
        .get_or_init(|| {
            ...
            map.insert("development", serde_json::from_str(DEVELOPMENT_JSON).unwrap());
